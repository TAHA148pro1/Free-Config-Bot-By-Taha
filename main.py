...
CONFIG = {
    ...
    "host": (
        ...
        or os.environ.get("RAILWAY_STATIC_URL", "").strip().replace("https://", "").replace("http://", "").split("/", 1)[0]
        or os.environ.get("PUBLIC_BASE_URL", "").strip().replace("https://", "").replace("http://", "").split("/", 1)[0]
        or "localhost"
    ),
...
def _is_local_host(host: str | None) -> bool:
    ...
    return value in {"localhost", "127.0.0.1", "0.0.0.0", "::1"} or value.startswith("127.")


def get_host(
    request: Request | None = None,
) -> str:
    # اولویت اول: آدرس عمومی صریحی که در تنظیمات پنل ثبت شده.
    # مقدارهای قدیمی مثل localhost نباید بتوانند لینک‌های جدید را خراب کنند؛
    # در این حالت Resolver به Railway/Public Base fallback می‌کند.
    _, override_host = _split_base_url(CONFIG.get("public_base_url"))
    if override_host and not _is_local_host(override_host):
    ...
    if railway_domain:
        railway_host = railway_domain.replace("https://", "").replace("http://", "").split("/", 1)[0].split(":")[0]
        if not _is_local_host(railway_host):
            return railway_host

    env_base = os.environ.get("PUBLIC_BASE_URL", "").strip()
    if env_base:
        _, env_host = _split_base_url(env_base)
...
def public_base(host: str | None = None) -> str:
    """آدرس پایه‌ی عمومی پنل، مثل https://panel.example.com (بدون / انتهایی)."""
    return f"{get_scheme()}://{host or get_host()}"


def subscription_url_for_uid(uid: str, host: str | None = None) -> str:
    """تک‌منبعِ حقیقت برای «لینک ساب» یک کانفیگ.

    قبلاً این رشته در چند نقطه (get_link_info، لیست لینک‌های API، ربات) به‌صورت
    دستی ساخته می‌شد و امکان واگرایی داشت. هر مصرف‌کننده‌ای — از جمله ربات
    تلگرام — باید از همین تابع استفاده کند تا هیچ سیستم ساب موازی‌ای شکل نگیرد.
    """
    uid = str(uid or "").strip()
    if not uid:
        return ""
    return f"{public_base(host)}/sub/{uid}"


def info_url_for_uid(uid: str, host: str | None = None) -> str:
    """لینک صفحه‌ی اطلاعات/مصرف همان کانفیگ (همان منبع، همان دامنه)."""
    uid = str(uid or "").strip()
    if not uid:
        return ""
    return f"{public_base(host)}/info/{uid}"
...
def _bot_settings_snapshot() -> dict:
    ...
    try:
        if _BOTSYS_READY:
            running = bool(_bot_runner.status().get("running"))
    except Exception:
        running = False

    return {
        "bot_token": str(CONFIG.get("bot_token") or "").strip(),
        "admin_ids": str(CONFIG.get("bot_admin_ids") or "").strip(),
        "running": running,
    }
...
def get_link_info(
    ...
    return {
        ...
        "sub": subscription_url_for_uid(uid, host),
        "info": info_url_for_uid(uid, host),
...
async def load_state():
    ...
    try:
        ...
        # تنظیمات قالب Remark هم بخشی از State پایدار پنل هستند.
        for _flag in ("sub_remark_show_name", "sub_remark_show_volume", "sub_remark_show_id", "sub_remark_show_inbound"):
            if _flag in settings_data:
                CONFIG[_flag] = bool(settings_data.get(_flag))
        # توکن ذخیره‌شده در تنظیمات پنل (fallback وقتی Environment ست نشده).
        CONFIG["bot_token"] = str(settings_data.get("bot_token") or "").strip()
        CONFIG["bot_admin_ids"] = str(settings_data.get("bot_admin_ids") or "").strip()

        try:  # سازگاری با ماژول قدیمی (ابزارهای پیشرفته)
            import telegram_bot
        ...
        # زیرسیستم ربات تلگرام: خواندن State + اجرای Migration ها روی همان فایل.
        try:
            from botsys import store as _bot_store
            _bot_store.import_state(data, logger)
            logger.info(
                "Telegram subsystem state loaded (schema v%d): %d users / %d configs",
                _bot_store.schema_version(),
                len(_bot_store.TG_USERS),
                len(_bot_store.TG_USER_CONFIGS),
            )
        except Exception as bot_exc:
            logger.exception("Could not load telegram subsystem state: %s", bot_exc)
...
async def save_state():
    ...
    async with SAVE_LOCK:
        ...
        try:
            ...
            # زیرسیستم ربات تلگرام در همان فایل State ذخیره می‌شود (بدون فایل موازی).
            try:
                from botsys import store as _bot_store
                payload.update(_bot_store.export_state())
            except Exception as bot_exc:
                logger.warning("Could not export telegram subsystem state: %s", bot_exc)
...
async def shutdown():
    ...
    # ربات تلگرام قبل از بسته شدن سرویس تمیز متوقف می‌شود (صف Broadcast هم لغو می‌شود).
    try:
        await _tg_stop_bot()
    except Exception as exc:
        ...
        result.append(
            {
                **info,
                ...
                "sub_url":
                    subscription_url_for_uid(uid, host),

                "info_url":
                    info_url_for_uid(uid, host),
...
async def subscription_portal(uuid: str, request: Request):
    ...
    host = get_host(request)
    raw_url = subscription_url_for_uid(uuid, host)
    info_url = info_url_for_uid(uuid, host)
...
async def info_page(uid: str, request: Request):
    ...
    sub_url = subscription_url_for_uid(uid, host)
...
# زیرسیستم ربات تلگرام (botsys).
# مالک اتصال تلگرام این زیرسیستم است؛ ماژول قدیمی telegram_bot.py حفظ شده و
# Handler هایش به‌عنوان «ابزارهای پیشرفته» از پنل ادمین جدید در دسترس‌اند، ولی
# حلقه‌ی Polling قدیمی اجرا نمی‌شود تا خطای 409 Conflict تلگرام رخ ندهد.

try:

    from botsys import legacy as _bot_legacy
    from botsys import runner as _bot_runner
    from botsys.panel_api import router as _bot_panel_router
    ...
    logger.info(
        "Telegram subsystem routes loaded."
    )
    ...
    logger.exception(
        "Telegram subsystem unavailable: %s",
        exc,
    )


async def _tg_start_bot():
    """راه‌اندازی ربات. اگر توکن نباشد، بی‌صدا غیرفعال می‌ماند و پنل وب سالم کار می‌کند."""
    if not _BOTSYS_READY:
        return None

    status = await _bot_runner.start()
...
async def _tg_stop_bot():
    if not _BOTSYS_READY:
        return None
    _bot_legacy.detach()
    return await _bot_runner.stop()


async def _tg_restart_bot():
    if not _BOTSYS_READY:
        return None
    _bot_legacy.detach()
    status = await _bot_runner.restart()
    if status.get("running"):
        _bot_legacy.attach()
    return status
...
async def start_optional_telegram():
    ...
    try:
        ...
        status = await _tg_start_bot()

        if status and status.get("running"):
            logger.info(
                "Telegram bot running in %s mode.",
                status.get("mode"),
            )
        else:
            logger.info(
                "Telegram bot idle (no token configured or start failed)."
            )
...
@app.post(
    _bot_runner.WEBHOOK_PATH + "/{secret}" if _BOTSYS_READY else "/telegram/webhook/{secret}",
...
async def telegram_webhook(
    ...
    امنیت: هم Secret مسیر و هم هدر رسمی X-Telegram-Bot-Api-Secret-Token بررسی
    می‌شوند، پس کسی نمی‌تواند Update جعلی تزریق کند.

    همیشه 200 برمی‌گردانیم تا تلگرام Update را دوباره و دوباره نفرستد؛ خطاها
    داخل لاگ ثبت می‌شوند.
    """
    if not _BOTSYS_READY:
        return JSONResponse(
            {"ok": False},
    ...
    if not _bot_runner.verify_webhook_secret(secret, header_secret):
        logger.warning(
            "Rejected telegram webhook call with bad secret."
        )
        return JSONResponse(
            {"ok": False},
            status_code=403,
    ...
    try:
        await _bot_runner.handle_webhook_update(update)
    except Exception as exc:
        logger.exception(
            "Telegram webhook handling failed: %s",
            exc,
        )
...
async def api_update_settings(request: Request, token=Depends(require_owner)):
    ...
    try:
        ...
        if "bot_token" in body or "bot_admin_ids" in body:
            ...
            if new_token is not None:
                CONFIG["bot_token"] = str(new_token).strip()
            if new_admin_ids is not None:
                CONFIG["bot_admin_ids"] = str(new_admin_ids).strip()
            telegram_bot.configure(
    ...
    restarted = False
    try:
        if bot_settings_changed and _BOTSYS_READY:
            status = await _tg_restart_bot()
            restarted = bool(status and status.get("running"))
    except Exception as exc:
...
async def api_bot_start(token=Depends(require_owner)):
    try:
        ...
        if not (status and status.get("running")):
            detail = (status or {}).get("last_error") or "توکن ربات تنظیم نشده است"
            raise HTTPException(status_code=400, detail=f"ربات روشن نشد: {detail}")
    except HTTPException:
        raise
    except Exception as exc:
